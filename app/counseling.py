...
{{
  ...
  "improvement_points": [
    {{
      ...
      "body": "結論→理由→具体例を1段落の自然な文章に。最後に実際のセリフ「...」を必ず入れる"
    }}
  ],
...
improvement_points は最大2件。各 body は見出し・箇条書き・絵文字なしの自然な段落。
...
def _format_improvement_body(item: ItemEvaluation) -> str:
    """改善点を結論→理由→具体例の自然な段落に整える。"""
    reason = item.comment.strip() if item.comment else "文字起こしを聞くと、ここが弱く感じました。"
    phrase = _extract_example_phrase(item.next_action or "")
    if phrase:
        example = f"次回は「{phrase}」と聞く"
    else:
        example = "次回は具体セリフを入れて聞く"
    return _sanitize_tone(
        f"次回は{item.label}をもう一段入れると、お客様の状況が伝わりやすいと思います。"
        f"{reason}"
        f"{example}のがおすすめです。"
    )
...
def _normalize_improvement_body(body: str, title: str = "") -> str:
    ...
    if not body:
        theme = title or "ここ"
        return (
            f"次回は{theme}を意識すると、会話の質が上がると思います。"
            "文字起こし上、改善の余地がありました。"
            "次回は具体セリフを入れて聞くのがおすすめです。"
        )
    if "「" not in body and title:
        body = f"{body} たとえば、{title}に関する質問を入れるのがおすすめです。"
    return body
